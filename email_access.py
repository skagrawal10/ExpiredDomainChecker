	...
	for num in data[0].split():
		...
		if ifMailExist(Message_ID) == 0:
			try:
				...
				Message_Body=unicode(str(Message_Body), errors='replace')
				#udata.encode("ascii","ignore")
				#Subject = unicode(str(Subject), errors='replace')
				Subject = str(Subject);#.decode("utf-8")
				...
				#udata.encode("ascii","ignore")
				#Subject = unicode(str(Subject), errors='replace')
				#Subject = str(Subject);#.decode("utf-8")print "Error " + 
				#raise;
				print "Error in mail [Subject :" + Subject + "From " + From_Address;
				pass;
		else:
